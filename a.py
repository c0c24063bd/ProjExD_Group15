...
def reset_game():
    ...
    start_time = pygame.time.get_ticks() # 追加：リセット時に開始時刻を記録
...
while True:
    ...
    if game_state == "title":
        ...
        # 追加：残り時間の表示（右画面）
        time_txt = font.render(f"時間: {remaining_time}", True, (255, 0, 0) if remaining_time < 30 else (0,0,0))
        screen.blit(time_txt, (SCREEN_WIDTH - time_txt.get_width() - 10, 10))
